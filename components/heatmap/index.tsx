...
import { getGraphData, locales, GraphData, DayInfo } from "./records";
...
export const Heatmap: React.FC<HeatmapProps> = (props) => {
  ...
  }, [years, data, fullYear, i18n.locale]);
  ...
  useEffect(() => {
    if (
      ...
      (prevYears && prevYears.some((y) => !years.includes(y)))
    ) {
      fetchData();
    }
  ...
  function renderMonthLabels(monthLabels: MonthLabel[]) {
    const style = {
      fill: getTheme().text,
      fontSize,
    };
