import React, { cloneElement, useContext, ReactNode } from "react";
import {
  Avatar,
  Button,
  Box,
...
const SidebarButton = ({ icon, label, selected, ...rest }: SidebarButtonProps) => (
  <Button
    gap="medium"
    justify="start"
    fill
    ...
    return (
      <Nav gap="small">
        ...
        <Button
          ...
          onClick={() => push(inbox)}
          badge={
            (serverSession as any)?.inboxCount
              ? {
                  value: (serverSession as any)?.inboxCount,
                  background: {
...
function MobileSidebar(props: SidebarComponentProps) {
...
function DesktopSidebar(props: SidebarComponentProps) {
  const { serverSession, size, t } = props;
...
function Sidebar(props: SidebarComponentProps) {
