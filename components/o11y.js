...
function captureScriptData() {
  ...
  Array.prototype.forEach.call(document.scripts, (script) => {
    ...
    if (script.src) {
      ...
      filename = path.pathname.substring(path.pathname.lastIndexOf("/") + 1);
    } else {
      inlineCounter += 1;
    }
    ...
    return (data[filename] = {
      ...
      url: typeof path !== "undefined" ? path.href : "inline",
    });
  });
...
function reportLCP(metric) {
  ...
  if (lcp.element) {
    report.lcp_elementSelector = `${lcp.element.localName}.${[
      ...lcp.element.classList,
    ].join(".")}`;
  }
