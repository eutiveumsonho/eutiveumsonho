 ...
 *
 * @todo Make it more efficient by using a single query
 * @param {string} dreamId
 */
export async function hasAiCommentedOnDream(dreamId) {
  const hasCommented = await hasCommentedOnDream("Sonia", dreamId);
  const legacyHasCommented = await hasCommentedOnDream("Sonio", dreamId);
