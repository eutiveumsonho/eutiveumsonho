...
export async function createAnonymousDream(dreamData) {
  const body = JSON.stringify(dreamData);

  try {
    const response = await fetch("/api/data/anonymous", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body,
    });
    ...
    logError(error, {
      ...
      component: "createAnonymousDream",
    });
    return { success: false };
  }
}

export async function claimAnonymousDreams(anonymousKeys) {
  const body = JSON.stringify({ anonymousKeys });

  try {
    const response = await fetch("/api/data/claim-anonymous", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body,
    });
    ...
    logError(error, {
      ...
      component: "claimAnonymousDreams",
    });
    return { success: false };
  }
}
